...
public class RepStatsAction extends BaseAction {
	...
	public String doAdd() {
		try {
			RepStatsImpl repstatsImpl = new RepStatsImpl(inputDate, statis1,
					statis2, statis3, statis4, statis5, statis6, userId, rpi,
					rank, problem, param2, param1);
	...
	public String doUpdate() {
		try {
			RepStatsImpl repstatsImpl = new RepStatsImpl(sno, inputDate,
					...
					userId, rpi, rank, problem, param2, param1);
	...
	public enum ExportFiled {
		SNO("流水号"), INPUTDATE("输入日期"), STATIS1("统计数据1"), STATIS2("统计数据2"), STATIS3(
				"统计数据3"), STATIS4("统计数据4"), STATIS5("统计数据5"), STATIS6("统计数据6"), USERID(
				"统计用户"), RPI("rpi数值"), RANK("排名"), PROBLEM("问题"), PARAM2(
				"额外参数2"), PARAM1("额外参数1");
		private String str;
	...
	private Map<RepStatsSearchFields, Object> getCriterias() {
		...
		if (getInputDate() != null)
			criterias.put(RepStatsSearchFields.INPUTDATE,
					DateUtil.toString(getInputDate(), "yyyy-MM-dd"));
		if (getUserId() != null && getUserId() != 0)
			criterias.put(RepStatsSearchFields.USERID, getUserId());
