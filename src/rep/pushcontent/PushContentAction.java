...
public class PushContentAction extends BaseAction {
	...
	public String doAdd() {
		try {
			PushContentImpl pushcontentImpl = new PushContentImpl(userIds ,content ,pushTime ,userNames ,deviceType );
	...
	public String doUpdate() {
		try {
			PushContentImpl pushcontentImpl = new PushContentImpl( sno , userIds , content , pushTime , userNames , deviceType );
	...
	public enum ExportFiled {
		  SNO("流水号"),  USERIDS("推送用户id"),  CONTENT("推送内容"),  PUSHTIME("推送时间"),  USERNAMES("推送用户"),  DEVICETYPE("推送机器类型");
		private String str;
	...
	public String export() {
		...
		response.addHeader("Content-Disposition","attachment;filename=PushContentList.xls");
		...
		Collection<PushContent> pushcontentList = pMgr.searchPushContent(criterias, realOrderField(),
	...
	public String query() {
		...
		Collection<PushContent> moneyList = pMgr.searchPushContent(criterias, realOrderField(),
				startIndex, numPerPage);
	...
	private Map<PushContentSearchFields, Object> getCriterias() {
		Map<PushContentSearchFields, Object> criterias = new HashMap<PushContentSearchFields, Object>();
			...
			if (getDeviceType()!=null&&!"".equals(getDeviceType())&&!"-1".equals(getDeviceType())&&!"-2".equals(getDeviceType()))
